...
for (let index = 0; index < courses.length - 1; index += 1) {
  ...
  for (const repository of studentRepositories) {
    ...
    results.push({ student, realName: enrollment.realName, passed: current.course, score, unlocked: next.course, ...outcome });
    repositoryNames.add(outcome.repository);
  }
}

const lines = results.map((item) => `| ${item.realName} (@${item.student}) | ${item.passed} | ${item.score}/100 | ${item.unlocked} | ${item.action} | ${item.protection || 'not applicable'} |`).join('\n');
const summary = `# Curriculum progression

Mode: **${apply ? 'APPLY' : 'DRY RUN'}** · Organization: **${organization}**

| Learner | Passed | Score | Next course | Result | Main protection |
| --- | --- | ---: | --- | --- | --- |
